...
export class BasicMelee extends Skill {
	...
	public OnConstructServer(): void {
		...
		ParticleGroupManager.ParentParticleEmtters(this.ActivationParticles, this.Character.Instance.FindFirstChild("RightGripAttachment",true) as Instance);
		...
		this.HitPartConnection = this.SkillHitPart.Touched.Connect((hit) => {
			Logger.Log("BasicMelee", "Hit Part Touched");
			const damageContainer = this.CreateDamageContainer(111);
	...
	public OnStartServer() {
		...
		AnimationManager.PlayAnimation(this.ActivationAnimationTrack as AnimationTrack, BasicMelee.SkillConfiguration.ActivationTime);
